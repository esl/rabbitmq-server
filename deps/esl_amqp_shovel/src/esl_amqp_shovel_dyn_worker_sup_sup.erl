...
start_child({VHost, ShovelName} = Name, Def) ->
    esl_log_shovel:debug("Asked to start a dynamic Shovel named '~ts' in virtual host '~ts'", [ShovelName, VHost]),
    LockId = esl_amqp_shovel_locks:lock(Name),
    cleanup_specs(),
    esl_log_shovel:debug("Starting a mirrored supervisor named '~ts' in virtual host '~ts'", [ShovelName, VHost]),
...
stop_child({VHost, ShovelName} = Name) ->
    esl_log_shovel:debug("Asked to stop a dynamic Shovel named '~ts' in virtual host '~ts'", [ShovelName, VHost]),
    LockId = esl_amqp_shovel_locks:lock(Name),
